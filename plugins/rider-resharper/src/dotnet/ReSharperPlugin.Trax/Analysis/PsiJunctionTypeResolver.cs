...
namespace ReSharperPlugin.Trax.Core.Analysis
{
    /// <summary>
    /// Resolves IStep&lt;TIn, TOut&gt; type arguments from a step class using ReSharper PSI.
    /// Port of Trax.Core.Analyzers.Analysis.JunctionTypeResolver (Roslyn) to PSI.
    /// </summary>
    internal static class PsiJunctionTypeResolver
    {
        private const string IStepShortName = "IJunction";
        private const string IStepNamespace = "Trax.Core.Junction";
        ...
        /// </summary>
        public static StepTypes Resolve(ITypeElement junctionTypeElement)
        {
            foreach (var superType in junctionTypeElement.GetAllSuperTypes())
            {
