...
using ReSharperPlugin.Trax.Core.Analysis;

namespace ReSharperPlugin.Trax.Core.Hints
{
    [ElementProblemAnalyzer(
        typeof(IInvocationExpression),
        HighlightingTypes = new[] { typeof(ChainJunctionInlayHint) }
    )]
    public class ChainJunctionHintAnalyzer : ElementProblemAnalyzer<IInvocationExpression>
    {
        ...
        )
        {
            ...
            var junctionType = typeArgumentList.TypeArguments[0];
            var junctionTypeElement = junctionType.GetTypeElement<ITypeElement>();
            if (junctionTypeElement == null)
                return;

            // 6. Resolve IJunction<TIn, TOut> from the step type
            var resolved = PsiJunctionTypeResolver.Resolve(junctionTypeElement);
            if (resolved == null)
                return;
            ...
            var tooltip = junctionTypeElement.ShortName + ": " + tInLong + " \u2192 " + tOutLong;
            ...
            consumer.AddHighlighting(
                new ChainJunctionInlayHint(hintText, element, offset, tooltip)
            );
        }
    }
}
